    ...
    const fetchContactInfo = async () => {
      try {
        ...
        if (typeof response.data === "object" && Array.isArray(response.data.data)) {
          const mappedData = response.data.data.map( (data: { bank: { bank_name: string }; bank_id: number; currency: {name: string}; currency_id: number}) => ({
              ...data,
              bank_id: data.bank ? data.bank.bank_name : data.bank_id,
              currency_id: data.currency ? data.currency.name : data.currency_id
            }),
          );
