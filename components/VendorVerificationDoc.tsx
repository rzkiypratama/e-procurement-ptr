...
const VendorVerificationDoc = (props: Props) => {
    ...
    const getDocsVendor = async () => {
        setIsLoading(true)
        try {
            const response = await axios.get(`https://vendor.eproc.latansa.sch.id/api/verifikator/vendor-attachments/${props.vendorId}`, {
                headers: {
    ...
    const verifiedDoc = async (record: AttachmentDoc) => {
        ...
        try {
            ...
            const response = await axios.post(`https://vendor.eproc.latansa.sch.id/api/verifikator/vendor-attachments/${props.vendorId}/${record.id}`, body, {
                headers: {
                    "Authorization": "Bearer 366|RSq8PgJAx7JEGhAK5tayWacrkWMtEMtmyDc8hrDwc61803d5"
                }
            });
